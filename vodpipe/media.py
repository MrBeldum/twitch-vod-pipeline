...
def has_video_stream(tools: Tools, path: Path) -> bool:
    """Does this file hold a video stream ffprobe can read?

    ffmpeg returning zero is not proof that a usable file was written -- a cut can
    exit clean having produced nonempty junk. Anything that authorises calling an
    export a success checks the file, not the exit code.
    """
    try:
        probe = ffprobe_json(tools.ffprobe, path)
    except Exception:
